...
Conventional lightweight deep learning approaches often struggle to capture dynamic operational characteristics under resource-limited computational conditions. Recently, Mamba has emerged as a promising lightweight architecture and demonstrated notable success across multiple domains, owing to its inherent capability in modeling dynamic dependencies. However, its state-space mechanism exhibits high sensitivity to intense disturbances, undermining model robustness for industrial anomaly diagnosis. To address this issue, a novel robust Mamba (RMamba) framework is proposed for reliable industrial anomaly diagnosis under strong disturbance conditions. Specificallly, an
attention Mamba module is designed to jointly extract dynamic and static features. Then a lightweight global feature extraction module is built to re-calibrate global representations via channelwise adaptive scaling and shifting, enabling holistic feature perception with negligible computational overhead. Meanwhile, a dynamic gating module with adaptive learning that performs convolutional sparsification on global features is constructed to simultaneously enhance computational efficiency and improve robustness under lightweight design constraints. Extensive experiments conducted on a public conveyor belt video dataset and an industrial smelting furnace validate the efficacy of the proposed method.
